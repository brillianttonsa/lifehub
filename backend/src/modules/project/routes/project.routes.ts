...
router.patch(
  '/:id',
  loadProjectRole,
  requireCapability(can.manageProject, 'Only the owner can update a project'),
...
router.get(
  '/:projectId/entries',
  loadProjectRole,
  asyncHandler(EntryController.list),
);
router.post(
  '/:projectId/entries',
  loadProjectRole,
  requireCapability(
    can.writeEntry,
    'You do not have permission to write entries',
  ),
...
router.delete(
  '/:projectId/entries/:entryId',
  loadProjectRole,
  asyncHandler(EntryController.remove),
);
