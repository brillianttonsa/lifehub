    ...
    <Panel title="By wallet type" icon={WalletCards}>
      <div className="space-y-3">
        {(Object.keys(walletTypeLabels) as WalletType[]).map((type) => (
          <div key={type} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-3 text-sm dark:bg-slate-950">
            <span className="shrink-0 font-medium text-slate-600 dark:text-slate-400">{walletTypeLabels[type]}</span>
            <span className="truncate text-right font-semibold text-slate-900 dark:text-slate-100">
