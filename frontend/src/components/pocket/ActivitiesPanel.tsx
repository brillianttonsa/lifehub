    ...
    <Panel title="Activities" icon={Tag} subtitle="Tag transactions by purpose">
      ...
      <form onSubmit={handleSubmit} className="mb-4 flex gap-2">
        <input
          ...
          className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none focus:border-indigo-300"
        />
        <button
          disabled={isSaving}
          ...
          >
            ...
            {activity.isDeleted ? (
              <button
                ...
                className="shrink-0 text-xs font-semibold text-emerald-700 hover:text-emerald-600"
              >
                Restore
              </button>
            ) : (
