    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10 bg-slate-50 text-slate-900">
        <div className="rounded-3xl border border-slate-200 bg-white shadow-lg p-10 max-w-xl text-center">
          ...
          <p className="text-sm text-slate-500 mb-6">Planning cycles are stored securely and require an authenticated LifeHub session.</p>
      ...
      <div className="max-w-[1440px] mx-auto">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-6">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">Planning cycles &amp; goals</h1>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <button onClick={() => setShowCreateCycle(true)} className="inline-flex items-center gap-2 rounded-full bg-indigo-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/10 transition hover:bg-indigo-500">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm mb-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-semibold">Planning cycles</h2>
              <p className="text-sm text-slate-500">Filter and sort your cycles, then open one to manage its goals.</p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                <Search size={16} className="text-slate-400" />
                <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search cycles" className="w-full bg-transparent text-slate-900 outline-none placeholder:text-slate-400" />
              </div>
              <select className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" value={status} onChange={(event) => setStatus(event.target.value)}>
                ...
                {cycleStatusOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
              </select>
              <select className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900" value={type} onChange={(event) => setType(event.target.value)}>
                <option value="">All types</option>
                {cycleTypeOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
              </select>
        ...
        <div className={`${viewMode === 'grid' ? 'grid gap-4 sm:grid-cols-2' : 'space-y-4'}`}>
          {cycles.length === 0 ? (
            ...
            cycles.map((cycle) => {
              ...
              return (
                <motion.article key={cycle.id} layout className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-2">
                      <p className="text-xs uppercase tracking-[0.24em] text-slate-400">{cycle.type}</p>
                      <h3 className="text-xl font-semibold text-slate-900">{cycle.name}</h3>
                      <p className="text-sm text-slate-500">{formatDate(cycle.startDate)} — {formatDate(cycle.endDate)}</p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-3xl bg-slate-50 p-3 text-sm text-slate-600">
                      <p className="text-[10px] uppercase tracking-[0.24em] text-slate-400">Goals</p>
                      <p className="mt-2 font-semibold text-slate-900">{cycle.completedGoalCount} / {cycle.goalCount} completed</p>
                    </div>
                    <div className="rounded-3xl bg-slate-50 p-3 text-sm text-slate-600">
                      <p className="text-[10px] uppercase tracking-[0.24em] text-slate-400">Overall progress</p>
                      <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-slate-200">
                        <div className="h-full bg-indigo-600 transition-all" style={{ width: `${cycle.progress}%` }} />
                      </div>
                      <p className="mt-1 text-xs font-semibold text-slate-900">{cycle.progress}%</p>
                    </div>
                  </div>

                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    <button onClick={() => setExpandedCycleId(isExpanded ? null : cycle.id)} className="inline-flex items-center gap-1.5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition">
                      {isExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />} {isExpanded ? 'Hide goals' : 'View goals'}
                    </button>
                    <button onClick={() => setShowAddGoalFor(cycle.id)} className="inline-flex items-center gap-1.5 rounded-2xl border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100 transition">
                      <Plus size={14} /> Add goal
                    </button>
                    <button onClick={() => handleArchiveCycle(cycle.id)} className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-700 hover:bg-amber-100 transition">
                      <Archive size={14} className="inline mr-1.5 -mt-0.5" /> Archive
                    </button>
                    <button onClick={() => handleDeleteCycle(cycle.id)} className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-100 transition">
                  ...
                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-5 overflow-hidden border-t border-slate-100 pt-4">
                        {cycle.goals.length === 0 ? (
                          <p className="text-sm text-slate-500">No goals in this cycle yet.</p>
                        ) : (
                              ...
                              <div key={goal.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                ...
                                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                  <div className="w-full sm:max-w-xs">
                                    <label className="text-[10px] uppercase tracking-[0.24em] text-slate-400">Progress ({goal.progress}%)</label>
                                    <input type="range" min={0} max={100} value={goal.progress} onChange={(event) => handleGoalProgressChange(goal.id, Number(event.target.value))} className="mt-1 w-full" />
                                  </div>
                                  <div className="flex flex-wrap items-center gap-2">
                                    <select value={goal.status} onChange={(event) => handleGoalStatusChange(goal.id, event.target.value as GoalStatus)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700">
                                      {goalStatusOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                                    </select>
                                    <button onClick={() => handleDeleteGoal(goal.id)} className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-100 transition">
              ...
              <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.96, opacity: 0 }} className="w-full max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-semibold text-slate-900">Create a planning cycle</h2>
                    <p className="mt-1 text-sm text-slate-500">A shared timeframe you'll group goals into, e.g. "Q1 2027" or "Ramadan Goals".</p>
                  </div>
                  <button onClick={() => setShowCreateCycle(false)} className="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-600 hover:bg-slate-100">Cancel</button>
                </div>
                <div className="mt-6 grid gap-4 lg:grid-cols-2">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select value={cycleForm.type} onChange={(event) => updateCycleForm('type', event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none">
                      {cycleTypeOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                    </select>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium uppercase tracking-wide text-slate-500">Status</label>
                    <select value={cycleForm.status} onChange={(event) => updateCycleForm('status', event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none">
                      {cycleStatusOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                    </select>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium uppercase tracking-wide text-slate-500">Start date</label>
                    <input type="date" value={cycleForm.startDate} onChange={(event) => updateCycleForm('startDate', event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium uppercase tracking-wide text-slate-500">End date</label>
                    <input type="date" value={cycleForm.endDate} onChange={(event) => updateCycleForm('endDate', event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none" />
                  </div>
                </div>
                {error && <p className="mt-4 text-sm text-rose-600">{error}</p>}
                <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
                  <button onClick={() => setShowCreateCycle(false)} className="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition">Cancel</button>
                  <button onClick={handleCreateCycle} disabled={isSaving} className="inline-flex items-center justify-center rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60">
          ...
          {showAddGoalFor && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
              <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.96, opacity: 0 }} className="w-full max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-2xl">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.24em] text-indigo-600">New goal</p>
                    <h2 className="mt-2 text-2xl font-semibold text-slate-900">Add a goal to this cycle</h2>
                  </div>
                  <button onClick={() => setShowAddGoalFor(null)} className="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-600 hover:bg-slate-100">Cancel</button>
                </div>
                <div className="mt-6 grid gap-4 lg:grid-cols-2">
                  <div className="lg:col-span-2">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select value={goalForm.priority} onChange={(event) => updateGoalForm('priority', event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none">
                      {goalPriorityOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                    </select>
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium uppercase tracking-wide text-slate-500">Status</label>
                    <select value={goalForm.status} onChange={(event) => updateGoalForm('status', event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none">
                      {goalStatusOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                    </select>
                  </div>
                  <div className="lg:col-span-2">
                ...
                {error && <p className="mt-4 text-sm text-rose-600">{error}</p>}
                <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
                  <button onClick={() => setShowAddGoalFor(null)} className="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition">Cancel</button>
                  <button onClick={() => showAddGoalFor && handleAddGoal(showAddGoalFor)} disabled={isSaving} className="inline-flex items-center justify-center rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60">
